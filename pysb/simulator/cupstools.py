...
class _CupsToolsSimulator(Simulator):

    _supports = {'multi_initials': False, 'multi_param_values': False}

    default_integrator_options = {}
    _integrator_options_allowed = frozenset()
    _file_prefix_supported = False

    def __init__(self, model, tspan=None, initials=None, param_values=None,
                 verbose=False, **kwargs):
        super(_CupsToolsSimulator, self).__init__(
            model, tspan=tspan, initials=initials, param_values=param_values,
    ...
    def _finalize_init(self, kwargs):
        ...
        self._running_time_regex = re.compile(r'[R|r]unning time:\s+(\d+\.\d+)')
    ...
    def run(self, tspan=None, initials=None, param_values=None):
        ...
        """
        super(_CupsToolsSimulator, self).run(tspan=tspan, initials=initials,
        ...
        # Create cupsTools general input files
        self._create_input_files(_cupsoda_infiles_dir)

        self._create_specific_input_files(_cupsoda_infiles_dir)

        # Build command
        # ./cupSODA input_model_folder blocks output_folder simulation_
        # file_prefix gpu_number fitness_calculation memory_use dump
        command = self._get_command(_cupsoda_infiles_dir)

        self._logger.info("Running " + self.integrator + ": " + ' '.join(command))
        ...
        if logger_level <= logging.INFO:
            ...
            if run_time_match:
                self._logger.info('{} reported time: {} seconds'.format(
                    self.integrator, run_time_match.group(1)))
        self._logger.debug(self.integrator + ' stdout:\n' + p_out)
        if p_err:
            self._logger.error(self.integrator + ' stderr:\n' + p_err)
        if p.returncode:
            raise SimulatorException(
                p_out.rstrip("at line") + "\n" + p_err.rstrip())
        tout, trajectories = self._load_trajectories(self._output_file_names())
        if self._cleanup:
            shutil.rmtree(self.outdir)
        end_time = time.time()
        self._logger.info("{} + I/O time: {} seconds".format(
            self.integrator, end_time - start_time))
    ...
    def _create_specific_input_files(self, directory):
        pass
    ...
    def _create_input_files(self, directory):
        ...
        with open(os.path.join(directory, "cs_vector"), 'w') as cs_vector:
            ...
            if self._obs_species_only:
                self._out_species = [False for _ in self._model.species]
            ...
            for j in range(self._len_rxns):
                ...
                for r in rate_args[j]:
                    if isinstance(r, pysb.Parameter):
                        ...
                        raise ValueError(
                            '{} does not currently support models with '
    ...
    def _load_trajectories(self, files):
        ...
        for n, filename in enumerate(files):
            ...
            if not os.path.isfile(filename):
                raise Exception("Cannot find output file " + filename)
...
class CupSodaSimulator(_CupsToolsSimulator):
    ...
    **kwargs: dict, optional
    ...
    1. Harris L.A., Nobile M.S., Pino J.C., Lubbock A.L.R., Besozzi D., Mauri
       G., Cazzaniga P., Lopez C.F., 2017. GPU-powered model analysis with
       PySB/cupSODA, Bioinformatics, 33(21), pp.3492-3494
    ...
    def _get_command(self, cupsoda_infiles_dir):
        # Path to cupSODA executable
        bin_path = get_path('cupsoda')

        return [bin_path, cupsoda_infiles_dir, str(self.n_blocks),
    ...
    def _output_file_names(self):
        ...
        for i in range(len(self.param_values)):
            fn = '{}_{}'.format(self._prefix, i)
            outfiles.append(os.path.join(self._cupsoda_outfiles_dir, fn))
...
class LassieSimulator(_CupsToolsSimulator):
    """An interface for running LASSIE, a CUDA implementation of LSODA.

    LASSIE is a graphics processing unit (GPU)-based implementation of the
    LSODA simulation algorithm optimized for large models.
    It requires an NVIDIA GPU card with support for the CUDA framework version
    7.5 or above. Further details of LASSIE and software can be found on github:
    https://github.com/aresio/LASSIE

    The simplest way to install LASSIE is to use a pre-compiled version,
    which can be downloaded from here:
    https://github.com/aresio/LASSIE/releases
    ...
    **kwargs: dict, optional
        ...
        * ``obs_species_only``: Only output species contained in observables
          (default: True)
        * ``cleanup``: Delete all temporary files after the simulation is
          finished. Includes both BioNetGen and LASSIE files. Useful for
          debugging (default: True)
        * ``prefix``: Prefix for the temporary directory containing LASSIE
          input and output files (default: model name)
        * ``base_dir``: Directory in which temporary directory with LASSIE
          input and output files are placed (default: system directory
          determined by `tempfile.mkdtemp`)
        * ``integrator``: Name of the integrator to use; see
          `default_integrator_options` (default: 'lassie')
        * ``integrator_options``: A dictionary of keyword arguments to
          supply to the integrator; see `default_integrator_options`.
    ...
    vol : float or None
        System volume
    outdir : str
        Directory where LASSIE output files are placed. Input files are
        also placed here.
    opts: dict
        Dictionary of options for the integrator in use.
    integrator : str
    ...
    1. If `vol` is defined, species amounts and rate constants are assumed
       to be in number units and are automatically converted to concentration
       units before generating the LASSIE input files. The species
       concentrations returned by LASSIE are converted back to number units
       during loading.

    2. If `obs_species_only` is True, only the species contained within
       observables are output by LASSIE. All other concentrations are set
       to 'nan'.

    References
    ----------

    1. Tangherloni A., Nobile M.S., Besozzi D., Mauri G., Cazzaniga P., 2017.
       LASSIE: simulating large-scale models of biochemical systems on GPUs,
       BMC Bioinformatics, 18(1), p.246.
    ...
    _integrator_options_allowed = {'max_steps', 'atol', 'rtol', 'vol'}

    def __init__(self, model, tspan=None, initials=None, param_values=None,
                 verbose=False, **kwargs):
        super(LassieSimulator, self).__init__(model, tspan=tspan,
                                              initials=initials,
                                              param_values=param_values,
                                              verbose=verbose, **kwargs)
        self.integrator = kwargs.pop('integrator', 'lassie')

        self._finalize_init(kwargs)

    def _get_command(self, lassie_infiles_dir):
        # Path to LASSIE executable
        bin_path = get_path('lassie')

        return [bin_path, lassie_infiles_dir, self._cupsoda_outfiles_dir]

    def _create_specific_input_files(self, directory):
        # c_vector
        with open(os.path.join(directory, "c_vector"), 'w') as c_matrix:
            cmtx = self._get_cmatrix()
            line = ""
            for j in range(self._len_rxns):
                if j > 0:
        ...
        # M_0
        with open(os.path.join(directory, "M_0"), 'w') as M_0:
            m0 = self.initials
            # if a volume has been defined, rescale populations
            # by N_A*vol to get concentration
            # (NOTE: act on a copy of self.initials, not
    ...
    def _output_file_names(self):
        return [os.path.join(self._cupsoda_outfiles_dir, 'output', 'Solution')]
