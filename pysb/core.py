...
def validate_init_dist(obj):
    if not isinstance(obj, str):
        raise TypeError('Input must be string')
    init_distributions = ['set_initial_condition_distribute_uniformly','set_initial_condition_place_near', \
                          'set_initial_condition_scatter']
    if obj not in init_distributions:
        raise ValueError(obj + ' ' + 'is not a possible initial distribution' )
...
#         return cset
    ...
    def initial(self, pattern, value, distribution=None, dist_arg = None):
        """
