...
#         return cset
    ...
    @property
    def odes(self):
        if not self._odes and self.reactions:
            odes_add = collections.defaultdict(list)
            odes_subtract = collections.defaultdict(list)
            for rxn in self.reactions:
                for p in rxn['products']:
                    odes_add[p].append(rxn['rate'])
                for r in rxn['reactants']:
                    odes_subtract[r].append(rxn['rate'])

            self._odes = [sympy.Add(*odes_add[i], evaluate=False) -
                          sympy.Add(*odes_subtract[i], evaluate=False) for i
                          in range(len(self.species))]
